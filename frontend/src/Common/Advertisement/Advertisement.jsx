...
const Advertisement = ({ setFlashAdd, flashAdd }) => {
  const [advertisementList, setAdvertisementList] = useState([]);
  const [advertisementSize, setAdvertisementSize] = useState([]);
  const getAdvertisementList = async () => {
    try {
      const response = await axiosClientServiceApi.get(`/advertisement/clientAdvertisement/`);
  ...
  const getAdvertisementSize = async () => {
    try {
      const response = await axiosClientServiceApi.get(`/advertisement/getclientAdvSize/`);
            ...
            <div>
              <div className="advertismentInfo">
                {advertisementList[0].title && (
                  <h3 className="p-2 px-0 m-0 fs-4 title">{advertisementList[0].title}</h3>
                )}
                {advertisementList[0].advertisement_description && (
                  <p className="description">{advertisementList[0].advertisement_description}</p>
                )}
                {advertisementList[0].phonen_number && (
                  <p className="phoneNumber">{advertisementList[0].phonen_number}</p>
                )}
              </div>
